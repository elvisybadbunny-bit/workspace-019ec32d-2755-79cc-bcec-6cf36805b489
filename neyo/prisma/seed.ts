...
async function main() {
  ...
  const dm = await createConversation(
    tenant.id,
    { id: teacher.id, fullName: "Njoroge Peter" },
  ...
  const group = await createConversation(
    tenant.id,
    { id: principal.id, fullName: "Wanjiru Kamau" },
  ...
  await sendMessage(tenant.id, { id: principal.id, fullName: "Wanjiru Kamau" }, {
    ...
    body: "Please submit Form 2 CATs by Friday.",
  });
  ...
  const deptNames = ["Languages", "Sciences", "Humanities", "Mathematics"];
  ...
  console.log("✓ Seeded B.4: 4 departments, 9 subjects (8-4-4), 3 terms (T2 current), 8 timetable slots + 1 lesson plan.");
